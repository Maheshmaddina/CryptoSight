    ...
    try:
        ...
        if task.ready():
            if task.successful():
                ...
                return JsonResponse({'status': 'FAILURE', 'error': str(task.result)}, status=500)
...
def prediction_history(request):
    ...
    # Run sync update for pending predictions whose target times have passed
    try:
        from .tasks import check_and_update_all_pending_predictions
        check_and_update_all_pending_predictions()
    except Exception as e:
        print(f"Error checking/updating pending predictions: {e}")
